...
export default function AuthShell({ eyebrow = 'Customer account', title, text, children, footer }) {
  return <main className="page-canvas grid min-h-screen bg-cream lg:grid-cols-[.9fr_1.1fr]">
    ...
    <section className="flex items-center justify-center p-6 py-12 sm:p-10"><div className="brutal-card w-full max-w-lg animate-rise p-7 sm:p-10"><Link to="/" className="mb-10 inline-flex items-center gap-2 text-sm font-bold text-muted hover:text-terracotta"><ArrowLeft size={16}/>Back to restaurant</Link><div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl border-[3px] border-ink bg-lime text-white shadow-[3px_3px_0_#171713]"><Sprout/></span><div><p className="text-xs font-black uppercase tracking-[.18em] text-lime">{eyebrow}</p><h1 className="mt-1 font-display text-5xl font-bold leading-none">{title}</h1></div></div>{text&&<p className="mt-5 text-sm leading-6 text-muted">{text}</p>}<div className="mt-8">{children}</div>{footer&&<div className="mt-7 text-center text-sm text-muted">{footer}</div>}</div></section>
  </main>
}

import { CalendarDays, CheckCircle2, ChevronDown, Clock3, Copy, Users } from 'lucide-react'
...
export default function Reserve() {
  const [form, setForm] = useState(initial); const [submitting, setSubmitting] = useState(false); const [confirmation, setConfirmation] = useState(null)
  ...
  async function submit(e) {
    ...
    try {
      ...
      const { data, error } = await client.rpc('create_reservation', { p_customer_name: form.customer_name.trim(), p_phone: form.phone.trim(), p_booking_date: form.booking_date, p_booking_time: form.booking_time, p_guests: Number(form.guests), p_table_number: form.table_number })
      if (error) throw error
      setConfirmation(Array.isArray(data) ? data[0] : data); setForm(initial); window.scrollTo({ top: 0, behavior: 'smooth' })
    } catch (error) { toast.error(error.message.includes('already reserved') ? 'This table is already reserved.' : error.message) }
  ...
  <section className="py-16"><form className="shell grid gap-6 lg:grid-cols-[.9fr_1.1fr]" onSubmit={submit}>
    ...
    <div className="card h-fit p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-widest text-lime">02 · Your details</p><h2 className="mt-2 font-display text-3xl">Who’s joining us?</h2>
      ...
      <div className="mt-7 grid gap-5"><label><span className="label">Customer name</span><input required minLength="2" maxLength="80" className="input" name="customer_name" value={form.customer_name} onChange={update} placeholder="Full name"/></label><label><span className="label">Phone number</span><input required inputMode="tel" className="input" name="phone" value={form.phone} onChange={update} placeholder="+91 98765 43210"/></label><label><span className="label">Number of guests</span><div className="relative"><Users className="absolute left-4 top-3.5 text-muted" size={17}/><select className="input pl-11" name="guests" value={form.guests} onChange={update}>{Array.from({length:12},(_,i)=>i+1).map(n=><option key={n} value={n}>{n} {n===1?'guest':'guests'}</option>)}</select></div></label>
      <div className="mt-2 rounded-xl bg-white/[.035] p-4 text-sm text-muted">{form.table_number?<span>Selected <strong className="text-cream">Table {form.table_number}</strong> for {form.booking_date} at {form.booking_time}</span>:<span>No table selected yet.</span>}</div><button className="btn-primary w-full" disabled={submitting||!isSupabaseConfigured}>{submitting?'Securing your table…':'Book table'}</button><p className="text-center text-[11px] leading-5 text-muted">By booking, you agree to arrive within 15 minutes of your reservation time.</p></div>
    </div>
  </form></section></>
}
